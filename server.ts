...
async function configureApp() {
  ...
  app.post("/api/petugas", authenticateToken, async (req: any, res: any) => {
    try {
      // Security: Only Super Admin should do this? We can check req.user.wilayah here
      if (req.user.wilayah !== "Super Admin") {
  ...
  // 3. Verify Username and Password (Secured with Hashing check)
  app.post("/api/auth/verify", async (req, res) => {
    try {
      ...
      if (result.success && result.wilayah) {
        // Generate dynamic 32-char session token
  ...
  // 4. Get Inspection Kriteria (Static, Offline-ready)
  ...
  // 5. Get List of Registered Places in a Work Area (for Inspeksi Form selection)
  ...
  app.get("/api/rekap", async (req, res) => {
    try {
      ...
      // Format logs output with Bulan_Kegiatan and formatted Timestamp
  ...
  // 3b. Reset Password for Officer (Super Admin only)
  app.post("/api/auth/reset-pin", authenticateToken, async (req: any, res) => {
    try {
      ...
      // Ensure the logged in user is actually the Super Admin
  ...
  // 7. Register New Tempat / Master Location (Protected & Sanitized)
  ...
  // 7a. Update Existing Tempat (Protected & Sanitized)
  ...
  // 7b. Delete Inactive Tempat (Protected)
  ...
  // 7c. Get Change Logs Audit Trail (Protected)
  ...
  // 8. Save Inspection Result (Protected & Sanitized)
